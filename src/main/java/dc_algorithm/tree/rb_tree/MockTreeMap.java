...
import dc_algorithm.tree.rb_tree.RBTree.Node;

public class MockTreeMap<K, V> extends AbstractMap<K, V>
     ...
     */
    public MockTreeMap(Comparator<? super K> comparator) {
     ...
     */
    public MockTreeMap(Map<? extends K, ? extends V> m) {
        comparator = null;
        putAll(m);
    }
     ...
     * @throws NullPointerException if the specified map is null
     */
    public MockTreeMap(SortedMap<K, ? extends V> m) {
	...
	/**
	 * Removes the mapping for this key from this MockTreeMap if present.
	 *
	 * @param key
	...
	/**
	 * Returns a shallow copy of this {@code MockTreeMap} instance. (The keys and
	...
	class Values extends AbstractCollection<V> {
		...
		public int size() {
			return MockTreeMap.this.size();
		}

		public boolean contains(Object o) {
			return MockTreeMap.this.containsValue(o);
		}
		...
		public void clear() {
			MockTreeMap.this.clear();
		}

		public Spliterator<V> spliterator() {
			return new ValueSpliterator<K, V>(MockTreeMap.this, null, null, 0, -1, 0);
		}
	}
	...
	class EntrySet extends AbstractSet<Map.Entry<K, V>> {
		...
		public Spliterator<Map.Entry<K, V>> spliterator() {
			return new EntrySpliterator<K, V>(MockTreeMap.this, null, null, 0, -1, 0);
		}
	}
	...
	static final class KeySet<E> extends AbstractSet<E> implements NavigableSet<E> {
		...
		public Iterator<E> iterator() {
			if (m instanceof MockTreeMap)
				return ((MockTreeMap<E, ?>) m).keyIterator();
			else
				return ((MockTreeMap.NavigableSubMap<E, ?>) m).keyIterator();
		}

		public Iterator<E> descendingIterator() {
			if (m instanceof MockTreeMap)
				return ((MockTreeMap<E, ?>) m).descendingKeyIterator();
			else
				return ((MockTreeMap.NavigableSubMap<E, ?>) m).descendingKeyIterator();
		}
	...
	/**
	 * Base class for MockTreeMap Iterators
	 */
	...
	/**
	 * Compares two keys using the correct comparison method for this MockTreeMap.
	 */
	@SuppressWarnings("unchecked")
	 ...
	 */
	static <K, V> Map.Entry<K, V> exportEntry(MockTreeMap.Entry<K, V> e) {
	 ...
	 */
	static <K, V> K keyOrNull(MockTreeMap.Entry<K, V> e) {
	...
	abstract static class NavigableSubMap<K, V> extends AbstractMap<K, V>
		...
		NavigableSubMap(MockTreeMap<K, V> m, boolean fromStart, K lo, boolean loInclusive, boolean toEnd, K hi,
				boolean hiInclusive) {
			if (!fromStart && !toEnd) {
		...
		final MockTreeMap.Entry<K, V> absLowest() {
			MockTreeMap.Entry<K, V> e = (fromStart ? m.getFirstEntry()
					: (loInclusive ? m.getCeilingEntry(lo) : m.getHigherEntry(lo)));
			return (e == null || tooHigh(e.key)) ? null : e;
		}

		final MockTreeMap.Entry<K, V> absHighest() {
			MockTreeMap.Entry<K, V> e = (toEnd ? m.getLastEntry()
					: (hiInclusive ? m.getFloorEntry(hi) : m.getLowerEntry(hi)));
			return (e == null || tooLow(e.key)) ? null : e;
		}

		final MockTreeMap.Entry<K, V> absCeiling(K key) {
			if (tooLow(key))
				return absLowest();
			MockTreeMap.Entry<K, V> e = m.getCeilingEntry(key);
			return (e == null || tooHigh(e.key)) ? null : e;
		}

		final MockTreeMap.Entry<K, V> absHigher(K key) {
			if (tooLow(key))
				return absLowest();
			MockTreeMap.Entry<K, V> e = m.getHigherEntry(key);
			return (e == null || tooHigh(e.key)) ? null : e;
		}

		final MockTreeMap.Entry<K, V> absFloor(K key) {
			if (tooHigh(key))
				return absHighest();
			MockTreeMap.Entry<K, V> e = m.getFloorEntry(key);
			return (e == null || tooLow(e.key)) ? null : e;
		}

		final MockTreeMap.Entry<K, V> absLower(K key) {
			if (tooHigh(key))
				return absHighest();
			MockTreeMap.Entry<K, V> e = m.getLowerEntry(key);
			return (e == null || tooLow(e.key)) ? null : e;
		}

		/** Returns the absolute high fence for ascending traversal */
		final MockTreeMap.Entry<K, V> absHighFence() {
			return (toEnd ? null : (hiInclusive ? m.getHigherEntry(hi) : m.getCeilingEntry(hi)));
		}

		/** Return the absolute low fence for descending traversal */
		final MockTreeMap.Entry<K, V> absLowFence() {
		...
		abstract MockTreeMap.Entry<K, V> subLowest();

		abstract MockTreeMap.Entry<K, V> subHighest();

		abstract MockTreeMap.Entry<K, V> subCeiling(K key);

		abstract MockTreeMap.Entry<K, V> subHigher(K key);

		abstract MockTreeMap.Entry<K, V> subFloor(K key);

		abstract MockTreeMap.Entry<K, V> subLower(K key);
		...
		public final Map.Entry<K, V> pollFirstEntry() {
			MockTreeMap.Entry<K, V> e = subLowest();
		...
		public final Map.Entry<K, V> pollLastEntry() {
			MockTreeMap.Entry<K, V> e = subHighest();
			Map.Entry<K, V> result = exportEntry(e);
			if (e != null)
				m.deleteEntry(e);
			return result;
		...
		public final NavigableSet<K> navigableKeySet() {
			...
			return (nksv != null) ? nksv : (navigableKeySetView = new MockTreeMap.KeySet<>(this));
		}
		...
		abstract class EntrySetView extends AbstractSet<Map.Entry<K, V>> {
			...
			public boolean isEmpty() {
				MockTreeMap.Entry<K, V> n = absLowest();
			...
			public boolean contains(Object o) {
				...
				if (!inRange(key))
					return false;
				MockTreeMap.Entry<?, ?> node = m.getEntry(key);
			...
			public boolean remove(Object o) {
				...
				if (!inRange(key))
					return false;
				MockTreeMap.Entry<K, V> node = m.getEntry(key);
		...
		abstract class SubMapIterator<T> implements Iterator<T> {
			MockTreeMap.Entry<K, V> lastReturned;
			MockTreeMap.Entry<K, V> next;
			final Object fenceKey;
			int expectedModCount;

			SubMapIterator(MockTreeMap.Entry<K, V> first, MockTreeMap.Entry<K, V> fence) {
			...
			final MockTreeMap.Entry<K, V> nextEntry() {
				MockTreeMap.Entry<K, V> e = next;
			...
			final MockTreeMap.Entry<K, V> prevEntry() {
				MockTreeMap.Entry<K, V> e = next;
				if (e == null || e.key == fenceKey)
					throw new NoSuchElementException();
				if (m.modCount != expectedModCount)
		...
		final class SubMapEntryIterator extends SubMapIterator<Map.Entry<K, V>> {
			SubMapEntryIterator(MockTreeMap.Entry<K, V> first, MockTreeMap.Entry<K, V> fence) {
				super(first, fence);
			}
		...
		final class DescendingSubMapEntryIterator extends SubMapIterator<Map.Entry<K, V>> {
			DescendingSubMapEntryIterator(MockTreeMap.Entry<K, V> last, MockTreeMap.Entry<K, V> fence) {
				super(last, fence);
			}
		...
		final class SubMapKeyIterator extends SubMapIterator<K> implements Spliterator<K> {
			SubMapKeyIterator(MockTreeMap.Entry<K, V> first, MockTreeMap.Entry<K, V> fence) {
				super(first, fence);
			}
		...
		final class DescendingSubMapKeyIterator extends SubMapIterator<K> implements Spliterator<K> {
			DescendingSubMapKeyIterator(MockTreeMap.Entry<K, V> last, MockTreeMap.Entry<K, V> fence) {
				super(last, fence);
			}
	...
	static final class AscendingSubMap<K, V> extends NavigableSubMap<K, V> {
		...
		AscendingSubMap(MockTreeMap<K, V> m, boolean fromStart, K lo, boolean loInclusive, boolean toEnd, K hi,
				boolean hiInclusive) {
		...
		MockTreeMap.Entry<K, V> subLowest() {
			return absLowest();
		}

		MockTreeMap.Entry<K, V> subHighest() {
			return absHighest();
		}

		MockTreeMap.Entry<K, V> subCeiling(K key) {
			return absCeiling(key);
		}

		MockTreeMap.Entry<K, V> subHigher(K key) {
			return absHigher(key);
		}

		MockTreeMap.Entry<K, V> subFloor(K key) {
			return absFloor(key);
		}

		MockTreeMap.Entry<K, V> subLower(K key) {
			return absLower(key);
		}
	}
	...
	static final class DescendingSubMap<K, V> extends NavigableSubMap<K, V> {
		...
		DescendingSubMap(MockTreeMap<K, V> m, boolean fromStart, K lo, boolean loInclusive, boolean toEnd, K hi,
				boolean hiInclusive) {
			super(m, fromStart, lo, loInclusive, toEnd, hi, hiInclusive);
		}
	 ...
	 * previous releases of MockTreeMap that did not support NavigableMap. It
	...
	private class SubMap extends AbstractMap<K, V> implements SortedMap<K, V>, java.io.Serializable {
		...
		private Object readResolve() {
			return new AscendingSubMap<>(MockTreeMap.this, fromStart, fromKey, true, toEnd, toKey, false);
		}
	...
	/**
	 * Returns the first Entry in the MockTreeMap (according to the MockTreeMap's
	 * key-sort function). Returns null if the MockTreeMap is empty.
	 */
	...
	/**
	 * Returns the last Entry in the MockTreeMap (according to the MockTreeMap's
	 * key-sort function). Returns null if the MockTreeMap is empty.
	 */
	 ...
	 */
	static <K, V> MockTreeMap.Entry<K, V> successor(Entry<K, V> t) {
		if (t == null)
			return null;
	...
	/**
	 * Save the state of the {@code MockTreeMap} instance to a stream (i.e.,
	 * serialize it).
	 *
	 * @serialData The <em>size</em> of the MockTreeMap (the number of key-value
	 *             mappings) is emitted (int), followed by the key (Object) and
	 *             value (Object) for each key-value mapping represented by the
	 *             MockTreeMap. The key-value mappings are emitted in key-order (as
	 *             determined by the MockTreeMap's Comparator, or by the keys'
	 *             natural ordering if the MockTreeMap has no Comparator).
	 */
	...
	/**
	 * Reconstitute the {@code MockTreeMap} instance from a stream (i.e.,
	 * deserialize it).
	 */
	 ...
	 *
	 * It is assumed that the comparator of the MockTreeMap is already set prior to
	 * calling this method.
	 *
	 * @param size
	 ...
	 * size fields of the MockTreeMap are already set prior to calling this method.
	...
	static <K> Spliterator<K> keySpliteratorFor(NavigableMap<K, ?> m) {
		if (m instanceof MockTreeMap) {
			@SuppressWarnings("unchecked")
			MockTreeMap<K, Object> t = (MockTreeMap<K, Object>) m;
			return t.keySpliterator();
		}
		if (m instanceof DescendingSubMap) {
			@SuppressWarnings("unchecked")
			DescendingSubMap<K, ?> dm = (DescendingSubMap<K, ?>) m;
			MockTreeMap<K, ?> tm = dm.m;
			if (dm == tm.descendingMap) {
				@SuppressWarnings("unchecked")
				MockTreeMap<K, Object> t = (MockTreeMap<K, Object>) tm;
	...
	static class TreeMapSpliterator<K, V> {
		final MockTreeMap<K, V> tree;
		MockTreeMap.Entry<K, V> current; // traverser; initially first node in range
		MockTreeMap.Entry<K, V> fence; // one past last, or null
		int side; // 0: top, -1: is a left split, +1: right
		int est; // size estimate (exact only for top-level)
		int expectedModCount; // for CME checks

		TreeMapSpliterator(MockTreeMap<K, V> tree, MockTreeMap.Entry<K, V> origin, MockTreeMap.Entry<K, V> fence, int side, int est,
				int expectedModCount) {
			this.tree = tree;
			this.current = origin;
			this.fence = fence;
	...
	static final class KeySpliterator<K, V> extends TreeMapSpliterator<K, V> implements Spliterator<K> {
		KeySpliterator(MockTreeMap<K, V> tree, MockTreeMap.Entry<K, V> origin, MockTreeMap.Entry<K, V> fence, int side, int est,
				int expectedModCount) {
		...
		public KeySpliterator<K, V> trySplit() {
			...
			int d = side;
			MockTreeMap.Entry<K, V> e = current, f = fence,
		...
		public void forEachRemaining(Consumer<? super K> action) {
			...
			if (est < 0)
				getEstimate(); // force initialization
			MockTreeMap.Entry<K, V> f = fence, e, p, pl;
	...
	static final class DescendingKeySpliterator<K, V> extends TreeMapSpliterator<K, V> implements Spliterator<K> {
		DescendingKeySpliterator(MockTreeMap<K, V> tree, MockTreeMap.Entry<K, V> origin, MockTreeMap.Entry<K, V> fence, int side,
		...
		public void forEachRemaining(Consumer<? super K> action) {
			...
			if (est < 0)
				getEstimate(); // force initialization
			MockTreeMap.Entry<K, V> f = fence, e, p, pr;
			if ((e = current) != null && e != f) {
				current = f; // exhaust
				do {
					action.accept(e.key);
	...
	static final class ValueSpliterator<K, V> extends TreeMapSpliterator<K, V> implements Spliterator<V> {
		ValueSpliterator(MockTreeMap<K, V> tree, MockTreeMap.Entry<K, V> origin, MockTreeMap.Entry<K, V> fence, int side, int est,
				int expectedModCount) {
			super(tree, origin, fence, side, est, expectedModCount);
		}
	...
	static final class EntrySpliterator<K, V> extends TreeMapSpliterator<K, V> implements Spliterator<Map.Entry<K, V>> {
		EntrySpliterator(MockTreeMap<K, V> tree, MockTreeMap.Entry<K, V> origin, MockTreeMap.Entry<K, V> fence, int side, int est,
				int expectedModCount) {
			super(tree, origin, fence, side, est, expectedModCount);
		}
	...
	public void displayTree() {
		...
		while (isRowEmpty == false) {
			...
			while (globalStack.isEmpty() == false) {
				Entry<K, V> temp = globalStack.pop();
				if (temp != null) {
